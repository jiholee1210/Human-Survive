...
public class EnemySpawner : MonoBehaviour
{
    ...
    private IEnumerator EnemySpawn() {
        ...
        // 2 웨이브 : 뱀 + 뱀 호드
        for(float timer = 0; timer < 120f; timer += 0.3f) {
            Spawn(enemyData[1]);
        ...
        // 3 웨이브 : 리자드
        for(float timer = 0; timer < 120f; timer += 0.7f) {
            Spawn(enemyData[2]);
        ...
        // 5 웨이브 : 선인장
        for(float timer = 0; timer < 120f; timer += 0.2f) {
            Spawn(enemyData[4]);
            yield return new WaitForSeconds(0.2f);
        }

        // 6 웨이브 : 박쥐 + 뱀 다수
        for(float timer = 0; timer < 120f; timer += 0.4f) {
            Spawn(enemyData[0]);
            Spawn(enemyData[1]);
            yield return new WaitForSeconds(0.4f);
        }
        ...
        // 8 웨이브 : 거북맨
        for(float timer = 0; timer < 120f; timer += 0.5f) {
            Spawn(enemyData[6]);
